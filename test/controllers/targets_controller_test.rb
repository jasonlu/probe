...
class TargetsControllerTest < ActionController::TestCase
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post :create, target: { interval: @target.interval, title: @target.title, url: @target.url, user_id: @target.user_id }
    end
  ...
  test "should update target" do
    patch :update, id: @target, target: { interval: @target.interval, title: @target.title, url: @target.url, user_id: @target.user_id }
    assert_redirected_to target_path(assigns(:target))
  end
